...
instruction_params = ((oneOf(declared_param)|oneOf(variables)|Word(nums)) + ZeroOrMore(Literal(",") + (oneOf(declared_param)|oneOf(variables)|Word(nums)))) | Empty()
...
try:
    result = var_declaration.parseString("vars var1, var2 ;")
    print("La cadena es válida según la gramática")
except Exception as e:
    print("La cadena no es válida según la gramática")
    print(e)
...
"""
try:
    result = procs.parseString("procs putcb [ |c , b | assignto : 1 , 2 ; put : c , chips ; put : b , balloons ]")
    print("La cadena es válida según la gramática")
